...
describe('CertifierService - createCertifier with CSV files', () => {
  let certifierService: CertifierService;
  ...
  beforeEach(async () => {
    ...
    certifierService = module.get<CertifierService>(CertifierService);
  ...
  it('should correctly process a well-formatted CSV file', () => {
    const csvFilePath = path.resolve(
      __dirname,
      '../../../test/csv-test/good-format/certifiers.csv',
    );
    const csvBuffer = fs.readFileSync(csvFilePath);
    const csvData = csvService.parseCSV(
      csvBuffer,
      certifierService['createCertifier'].bind(certifierService),
    );
  ...
  it('should throw an error for a badly formatted CSV file', async () => {
    const csvFilePath = path.resolve(
      __dirname,
      '../../../test/csv-test/wrong-format/certifiers.csv',
    );
    const csvBuffer = fs.readFileSync(csvFilePath);
